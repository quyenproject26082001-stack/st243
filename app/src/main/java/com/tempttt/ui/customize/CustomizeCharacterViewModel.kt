...
import com.tempttt.data.model.custom.ItemColorImageModel
import com.tempttt.data.model.custom.ItemColorModel
import com.tempttt.data.model.custom.ItemNavCustomModel
import com.tempttt.data.model.custom.LayerListModel
import com.tempttt.data.model.custom.NavigationModel
...
class CustomizeCharacterViewModel : ViewModel() {
    ...
    suspend fun setBottomNavigationListDefault(targetLayerIndex: Int = -1) {
        ...
        if (itemNavList.size > firstLayerIndex) {
            ...
            val hasNone = firstNavItems.firstOrNull()?.path == com.tempttt.core.utils.key.AssetsKey.NONE_LAYER
            val hasRandom = firstNavItems.firstOrNull()?.path == com.tempttt.core.utils.key.AssetsKey.RANDOM_LAYER
    ...
    fun checkDataInternet(context: Activity, action: (() -> Unit)) {
        ...
        InternetHelper.checkInternet(context) { result ->
            if (result == HandleState.SUCCESS) {
                ...
                val dialog = com.tempttt.dialog.YesNoDialog(
                    context,
