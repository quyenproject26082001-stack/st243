...
import com.avatar.maker.celebrity.data.model.custom.ItemColorImageModel
...
import com.avatar.maker.celebrity.data.model.custom.NavigationModel
...
class CustomizeCharacterViewModel : ViewModel() {
    ...
    suspend fun setBottomNavigationListDefault(targetLayerIndex: Int = -1) {
        ...
        if (itemNavList.size > firstLayerIndex) {
            ...
            val hasNone = firstNavItems.firstOrNull()?.path == com.avatar.maker.celebrity.core.utils.key.AssetsKey.NONE_LAYER
            val hasRandom = firstNavItems.firstOrNull()?.path == com.avatar.maker.celebrity.core.utils.key.AssetsKey.RANDOM_LAYER
